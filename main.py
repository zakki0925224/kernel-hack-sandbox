...
def create_sandbox(kernel_version: str, sandbox_name: str):
    ...
    if not os.path.exists(f"{sandbox_dir}/{ROOTFS_NAME}"):
        ...
        run_shell_cmd("find . | cpio -o --format=newc > ../rootfs.img", dir=f"{BUSYBOX_DIR}/_install")
...
def run_sandbox(name):
    ...
    run_shell_cmd(f"qemu-system-x86_64 -kernel ./{BZIMAGE_NAME} -initrd ./{ROOTFS_NAME} -append \"rdinit=/bin/sh\" -serial mon:stdio", dir=sandbox_dir)
